...
export interface UseFormReturn<Values extends Record<string, any>> {
  ...
  register: (
    ...
    onBlur: FocusEventHandler<UseFormElement>
    onChange: ChangeEventHandler<UseFormElement>
    ref: (node: UseFormElement | null) => void
  }
  /** The reset function */
...
export const useForm = <Values extends Record<string, any>>(
  ...
  const paramsRef = useRef(new Map<keyof Values, UseFormRegisterParams<Values>>())
  ...
  const read = <Name extends keyof Values>(name: Name): Values[Name] => {
  ...
  const runValidation = async (name?: (keyof Values)[] | keyof Values) => {
    ...
    let names: (keyof Values)[]
    if (name) names = Array.isArray(name) ? name : [name]
    else names = [...paramsRef.current.keys()]
  ...
  const register = (name: keyof Values, params?: UseFormRegisterParams<Values>) => {
    if (params) paramsRef.current.set(name, params)
    else paramsRef.current.delete(name)

    return {
      name: String(name),
      ref: (node: UseFormElement | null) => {
        if (!node) {
        ...
        if (validateOnMount) void runValidation()
  ...
  const handleSubmit =
    ...
    async (event?: BaseSyntheticEvent) => {
      ...
      const newTouched = {} as Record<keyof Values, boolean>
      for (const key of names) newTouched[key] = true
